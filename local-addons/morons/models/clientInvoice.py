...
class ClientInvoice(models.Model):
    ...
    def write(self, vals):
        if 'payment_status' in vals and not self.env.context.get('skip_task_update'):
            for clientInvoice in self:
                if clientInvoice.sales_order:
                    clientInvoice.sales_order.with_context(skip_invoice_update=True).write({'payment_status': vals['payment_status']})
    ...
    sales_order = fields.Many2many('merctrans.sale', string='Sales Order', required=True)
    ...
    def fields_get(self, allfields=None, attributes=None):
        ...
        if not user_in_morons_category and 'sales_order' in res:
            # Modify the domain for the 'sales_order' field
            company_id = self.env.user.company_id.id
            res['sales_order']['domain'] = [('user_ids.company_id', '=', company_id)]
    ...
    def _compute_issued_to(self):
        for record in self:
            if record.sales_order and record.sales_order.user_ids:
                record.issued_to = record.sales_order.user_ids[0].id
            else:
